...
df.add_system_transition(State.PROMPT_oolgoalalign_err1,State.PROMPT_oolstressfr1,r'[!"I see. However you manage your time, just remember that even if things do not go as planned, they will work out eventually."#school_n4_lil"How often do you find"#help_ss"overwhelming?"]')
df.add_system_transition(State.PROMPT_oolgoalalign_yes1,State.PROMPT_oolstressfr1,r'[!"Do not forget to stop and smell the flowers too!"#school_n4_much"How often do you find"#help_ss"overwhelming?"]')
df.add_system_transition(State.PROMPT_oolgoalalign_no1,State.PROMPT_oolstressfr1,r'[!"Then just take it easy, and focus on fulfilling one responsibility at a time!"#school_n4_lil".How often do u find"#help_ss"overwhelming?"]')
...
df.add_system_transition(State.PROMPT_oolgoalalign_yes2,State.PROMPT_oolstressfr2,r'[!"It must be hard to keep pushing yourself to stay focused on your goals. Hang in there!"#school_n4_much"How often do you find"#help_ss"overwhelming?"]')
df.add_system_transition(State.PROMPT_oolgoalalign_no2,State.PROMPT_oolstressfr2,r'[!"I like how you manage your time. Life gets boring if all one does is pursuing one goal after another."#school_n4_lil"How often do u find"#help_ss"overwhelming?"]')
...
df.add_system_transition(State.PROMPT_oolpast_bad3,State.PROMPT_oolstressfr3,r'[!"What happened in the past is just a small part of the marathon to become the person you want to be. Hang in there!"#school_n3_bad"How often do you feel stressed about"#help_ss"?"]')
...
df.add_system_transition(State.PROMPT_oolgoalalign_err3,State.PROMPT_oolstressfr3,r'[!"It sounds like you already have a good time management strategy. That is a great place to start!"#school_n4_lil"How often do you find"#help_ss"overwhelming?"]')
...
df.add_system_transition(State.PROMPT_oolgoalalign_no3,State.PROMPT_oolstressfr3,r'[!"I am quite spontaneous too. That is what makes us flexible and creative!"#school_n4_lil"How often do you find"#help_ss"overwhelming?"]')
#stressfreq->help1->help2->onlinelearning1->onelinelearning2->studyspot1->studyspot2->destress
