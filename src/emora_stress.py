...
df.add_user_transition(State.PROMPT0_savage, State.BREAKUP_SAV, r'<{[!#ONT(ontbreakup)]}>')
...
df.add_system_transition(State.SINGLE0_short2, State.SINGLE12, r'[!"Someone is more than ready to move on from the previous relationship. I am happy for you. How often are you feeling stressed about being single?"]')
...
df.add_system_transition(State.BREAKUP3_yes2, State.BREAKUP42, r'[!"If it does not work out this time, it probably will not in the future. But it is your choice. Dont forget to love yourself more. What activity do you like to do during quarantine?"]')
df.add_system_transition(State.BREAKUP3_no2, State.BREAKUP42, r'[!"You deserve someone better! Time to focus on self development. What activity do you like to do during quarantine?"]')
...
df.add_system_transition(State.COVID0_ERR2, State.COVID1, r'[!"If a virus can ruin your relationship, then this relationship is probably not worth maintaining. What activity do you do to relax during quarantine?"]')
...
df.add_system_transition(State.BREAKUP2_ERR2, State.BREAKUP32, r'[!"Last year my girlfriend broke up with me because of her insecurity around long distance. And I learned the importance of committment. Just curious, do you want to get back together ?"]')
df.add_system_transition(State.BREAKUP3_ERR2, State.BREAKUP42, r'[!"I think you are ready to move on! Time to focus self development. During quarantine, I really enjoy trying new stuff. What activity do you like to do during quarantine?"]')
...
df.add_system_transition(State.ROMAN0_short2, State.ROMAN12, r'[!"Still during the honeymoon period? That is so cute. How often do you feel stressed about your relationship?"]')
df.add_system_transition(State.ROMAN0_mid2, State.ROMAN12, r'[!"No wonder. I would be surprised if you have not encountered any issues in your relationship. How often do you feel stressed about your relationship?"]')
df.add_system_transition(State.ROMAN0_long2, State.ROMAN12, r'[!"Oh wow. I am in love with your love story. How often do you feel stressed about your relationship?"]')
df.add_user_transition(State.ROMAN12, State.ROMAN1_often2,
                       r'<{[!#ONT(ontoften)],/(?:\s|^)(((once|twice|thrice|three\stimes|four\stimes|five\stimes|1\stime|2\stimes|3\stimes|4\stimes|5\stimes)\s(every|per|a)(\sone|\s1|\stwo|\s2|\sthree|\s3|\sfour|\s4|\sfive|\s5|\sother|\ssix|\s6|\s7|\sseven|\seight|\s8|\s9|\snine|\sten|\s10)?\s(second+s?|sec+s?|min+s?|minute+s?|hour+s?|hr+s?|day+s?))|((once|twice|thrice|three\stimes|four\stimes|five\stimes|1\stime|2\stimes|3\stimes|4\stimes|5\stimes)\s(every|per|a)(\sone|\s1|\stwo|\s2|\sthree|\s3|\sother)?\s(week+s?))|((three\stimes|thrice|four\stimes|five\stimes|six\stimes|seven\stimes|3\stimes|4\stimes|5\stimes|6\stimes|7\stimes)\s(every|per|a)(\sone|\s1|\stwo|\s2|\sthree|\s3|\sother)?\s(month+s?))|((twice|two\stimes|2\stimes)\s(every|per|a)(\sone)?\s(month)))(?:\s|,|\.|$)/}>')
df.add_user_transition(State.ROMAN12, State.ROMAN1_sometimes2,
                       r'<{[!#ONT(ontsometimes)],/(?:\s|^)(((once|one\stime|two\stimes|twice|three\stimes|thrice|four\stimes|five\stimes|1\stime|2\stimes|3\stimes|4\stimes|5\stimes)\s(every|per|a)(\sone|\s1|\stwo|\s2|\sthree|\s3|\sfour|\s4|\sfive|\s5|\ssix|\s6|\sten|\s10|\sother)?\s(semester+s?|term+s?|quarter+s?|year+s?|decade+s?))|((once|1\stime|one\stime)\s(every|per|a)(\sone|\s1|\stwo|\s2|\sthree|\s3|\sfour|\s4|\sfive|\s5|\ssix|\s6|\sother)?\s(month+s?))|((once|1\stime|2\stimes|twice|thrice|3\stimes|three\stimes|one\stime|four\stimes|4\stimes)\s(every|a|per)(\sthree|\s3|\sfour|\s4|\sfive|\s5|\ssix|\s6|\sseven|\s7|\seight|\s8|\sten|\s10)\s(month+s?)))(?:\s|,|\.|$)/}>')
df.add_user_transition(State.ROMAN12, State.ROMAN1_never2, r'<{[!#ONT(ontnever)]}>')
...
df.add_system_transition(State.ROMAN1_never2, State.ROMAN22, r'[!"The stress in your relationship must be caused by the coronavirus right?"#low_n""]')
...
df.add_system_transition(State.ROMAN3_intimacy2, State.ROMAN42, r'[!"A relationship without intimacy is kinda similar to love at first sight. You should share some stressful and joyful things with each other! Have you ever talked about your stress with your boyfriend or girlfriend?"]')
df.add_system_transition(State.ROMAN3_passion2, State.ROMAN42, r'[!"A relationship without passion is kinda similar to a long-term marriage. Having satisfying sex can actually help a lot! Have you ever talked about your stress with your boyfriend or girlfriend?"]')
df.add_system_transition(State.ROMAN3_commitment2, State.ROMAN42, r'[!"A relationship without commitment is kinda similar to a romantic affair. You should make promises to each other! Have you ever talked about your stress with your boyfriend or girlfriend?"]')
...
df.add_system_transition(State.ROMAN4_yes2, State.ROMAN52, r'[!"Oh really? And it did not work out? Then I think you should do something relaxing to relieve your stress. What activity do you like to do during quanrantine?"#high_e""]')
df.add_system_transition(State.ROMAN4_no2, State.ROMAN52, r'[!"Perhaps effective communications with each other about your stress can strengthen your relationship. You should do something to relieve your stress. What activity do you like to do during quanrantine?"]')
...
df.add_system_transition(State.ROMAN0_ERR2, State.ROMAN12, r'[!"That is quite a while! You must been through a lot of stressful things already. How often do you feel stressed about your relationship?"]')
...
df.add_system_transition(State.ROMAN4_ERR2, State.ROMAN52, r'[!"Perhaps effective communications with each other about your stress can strengthen your relationship. You should do something to relieve your stress. What activity do you like to do during quanrantine?"]')
...
df.add_system_transition(State.COVID02, State.COVID1, r'[!"Oh...I am guessing this stress is coming from the fact that you cannot see each other. Not able to see my friends and talk about how our life suck is stressing me out. But doing some fun activities help me relax a lot. What activity do you like to do during quarantine?"]')
...
df.add_system_transition(State.SINGLE2_yes, State.SINGLE3, r'[!"Checkmate! It is not a bad thing to have a high standard, but it reduces probablity that the one that fits your standard will also like you. The struggle is real! What do you do to relax during quarantine?"]')
...
df.add_system_transition(State.SINGLE0_ERR, State.SINGLE1, r'[!"Its okay you are not alone, I have been single for a while too. How often do you feel stressed about being single?"]')
...
df.add_system_transition(State.ROMAN0_short, State.ROMAN1, r'[!"You must be still in your honeymoon phase. How often do you feel stressed about your relationship?"]')
df.add_system_transition(State.ROMAN0_mid, State.ROMAN1, r'[!"That is quite a while! I am assuming you guys are in a more certain and stable phase. How often do you feel stressed about your relationship?"]')
...
df.add_system_transition(State.ROMAN4_yes, State.ROMAN5, r'[!"Oh really? And it did not work out? Then I think you should do something relaxing to relieve your stress. What activity do you like to do during quanrantine?"#high_e""]')
df.add_system_transition(State.ROMAN4_no, State.ROMAN5, r'[!"Maybe effective communications with each other about your stress can strengthen your relationship. You should do something to relieve your stress. What activity do you like to do during quanrantine?"]')
...
df.add_system_transition(State.ROMAN0_ERR, State.ROMAN1, r'[!"Sounds like you guys are in a more certain and stable phase. How often do you feel stressed about your relationship?"]')
...
df.add_system_transition(State.ROMAN4_ERR, State.ROMAN5, r'[!"Maybe opening up to each other about things that your stress could help strengthen your relationship. You should do something to relieve your stress. What activity do you like to do during quanrantine?"]')
...
df.add_system_transition(State.COVID0, State.COVID1, r'[!"The quarantine situation does introduce a lot more unpredictability to our life. What activity do you do to relax during quarantine?"]')
...
df.add_system_transition(State.ROMAN0_short1, State.ROMAN11, r'[!"That is so cute. How often do you feel stressed about your relationship?"]')
...
df.add_system_transition(State.ROMAN3_intimacy1, State.ROMAN41, r'[!"A relationship without intimacy is going to be hard to maintain. You should share some stressful and joyful things with each other! Have you ever talked about your stress with your boyfriend or girlfriend?"]')
...
df.add_system_transition(State.ROMAN4_yes1, State.ROMAN51, r'[!"Oh really? And it did not work out? Then I think you should do something relaxing to relieve your stress. What activity do you like to do during quanrantine?"#high_e""]')
df.add_system_transition(State.ROMAN4_no1, State.ROMAN51, r'[!"Perhaps effective communications with each other about your stress could help strengthen your relationship. You should do something to relieve your stress. What activity do you like to do during quanrantine?"]')
...
df.add_system_transition(State.ROMAN4_ERR1, State.ROMAN51, r'[!"Perhaps effective communications with each other about your stress can strengthen your relationship. You should do something to relieve your stress. What activity do you like to do during quanrantine?"]')
...
df.add_system_transition(State.COVID1_food, State.COVID2, r'[!"Cook, eat, sleep, taking online classes, that bascially sums up my life now. Are you also taking online classes?"]')
...
df.add_system_transition(State.COVID2_yes, State.COVID3, r'[!"Nice! It feels so good to know someone is also suffering with me. How do you feel about remote learning?"]')
...
df.add_system_transition(State.COVID2_ERR, State.COVID3, r'[!"Nice! Good to know someone is also suffering with me. How do you feel about remote learning?"]')
