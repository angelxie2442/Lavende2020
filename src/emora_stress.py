...
df.add_user_transition(State.COVID01, State.COVID0_yes1, r'<[!#ONT(ontyes)]>')
df.add_user_transition(State.COVID01, State.COVID0_no1, r'<[!#ONT(ontno)]>')
df.add_system_transition(State.COVID0_yes1, State.COVID1, r'[!"It is nice to still be able to see each other! I think everyone must be super stressed under this chaotic situation... Honestly, I think doing some fun activities might reduce some of your stress. What activity do you like to do during quarantine?"]')
df.add_system_transition(State.COVID0_no1, State.COVID1, r'[!"Oh...I am guessing this stress is coming from the fact that you cannot see each other. Not able to see my friends and talk about how our life suck is stressing me out. But doing some fun activities help me relax a lot. What activity do you like to do during quarantine?"]')
...
df.add_system_transition(State.COVID0_yes, State.COVID1, r'[!" Honestly, I think doing some fun activities might reduce some of your stress. What activity do you like to do during quarantine?"]')
df.add_system_transition(State.COVID0_no, State.COVID1, r'[!"If you have faith in your love for each other, the quarantine situation will help you guys become an even stronger couple than before. But doing some fun activities help me relax a lot. What activity do you like to do during quarantine?"]')
