...
df = DialogueFlow(State.START, initial_speaker=DialogueFlow.Speaker.SYSTEM, kb=knowledge, macros={"positive":positive_score()})
'''
...
df.add_system_transition(State.PROMPT_MOOD_ERR, State.PROMPT_STRESSOR,
                         '"Umm...you know what? Recently, I am learning about stressful things. I will be happy if you can tell me something you are stressed about."')

df.add_user_transition(State.PROMPT_MOOD, State.GOOD_MOOD, "[#one($one)]")
...
df.add_system_transition(State.GOOD_MOOD, State.PROMPT_STRESSOR,
                         r'[! $one"I am glad you are feeling"$one"! Umm...you know what? Recently, I am learning about stressful things. I will be happy if you can tell me something you are stressed about."]')
...
# df.add_system_transition(State.SOCIAL0, State.SOCIAL1,
#                          r'[! "Yeah..."$STRESSOR_SOCIAL"is often stressful. Do you always feel stressed about it?"]')
#
# ###Yes_No_Error
...
# df.add_system_transition(State.SOCIAL3_ERR, State.SOCIAL_trauma_quit,
#                          r'[!"I mean you dont have to go to the"$STRESSOR_SOCIAL"if its making you uncomfortable. Do you have the option to not go?"]')
#
...
# df.add_system_transition(State.SOCIAL_trauma_loc_yes, State.SOCIAL_trauma_quit,
#                          r'[!"I mean you dont have to go to the"$STRESSOR_SOCIAL"if its making you uncomfortable. Do you have the option to not go?"]')
#
# ###Yes_No_Error
...
#
# df.add_system_transition(State.MUST, State.SOCIAL_activity,
#                          '"Ohh...Im so sorry you have to go through this, but I have faith in you! After all that, you should do something to relieve the stress. What activity do you like to do?"')
# df.add_system_transition(State.N_MUST, State.SOCIAL_activity,
#                          '"Exactly! Lets skip the stressful part and straight to the fun part. What do you like to do to relieve your stress?"')
#
# ###ERR
# df.set_error_successor(State.SOCIAL_activity, State.SOCIAL_activity_ERR)
# df.add_system_transition(State.SOCIAL_activity_ERR, State.SOCIAL_suggest_comedy,
#                          '"That sounds so interesting! I should try that next time. My strategy to cope with stress is...watch comedy! Do you like to watch comedy?"')
#
...
# df.add_system_transition(State.SOCIAL_activity, State.SOCIAL_suggest_comedy,
#                          r'[!" "$activity"sounds very stress relieving. Peronally I likes to watch comedies. Do you like comedy?"]')
#
# ########comedies
# df.add_user_transition(State.SOCIAL_suggest_comedy, State.SOCIAL_suggest_yes, r'<[!#ONT(ontyes)]>')
# df.add_system_transition(State.SOCIAL_suggest_yes, State.SOCIAL_comedian,
#                          '"Charlie Chaplin is my all-time favorite! I can laugh whole day just by looking at his face. How about you, whos your favorite comedian?"')
#
# ###ERR
# df.set_error_successor(State.SOCIAL_comedian, State.SOCIAL_comedian_ERR)
# df.add_system_transition(State.SOCIAL_comedian_ERR, State.SOCIAL_end,
#                          '"Well you know even comedians like them often feel stressed! So dont worry to much about feeling stressed in social situations!"')
#
# df.add_user_transition(State.SOCIAL_suggest_comedy, State.SOCIAL_suggest_no, r'<[!#ONT(ontno)]>')
# df.add_system_transition(State.SOCIAL_suggest_no, State.SOCIAL_end,
#                          r'[!"I see. Well. I hope talking to me make you feel better. Feel free to reach out to me again whenever you want to talk."]')
...
# df.add_system_transition(State.SOCIAL_comedian_name, State.SOCIAL_end,
#                          r'[!"Well you know even" $comedian "sometimes feel stressed and embarassed in social situations just like us, so dont let it bother you too much!"]')
#
...
# df.add_system_transition(State.SOCIAL_trauma_loc_no, State.SOCIAL_trauma_replay,
#                          '[!"That is good."$STRESSOR_SOCIAL"might not be that stressful in differnt environment. Do you think this time will still be stressful?"]')
...
# df.add_system_transition(State.SOCIAL_replay_yes, State.SOCIAL_neuroticism,
#                          r'[!"Umm...you never know! By the way, do you also feel stressed about other social situations?"]')
#
# ###########high neuroticism
# df.add_user_transition(State.SOCIAL_neuroticism, State.SOCIAL_neurotic_yes, r'<[!#ONT(ontyes)]>')
# df.add_system_transition(State.SOCIAL_neurotic_yes, State.SOCIAL_end,
#                          '"I am so proud of you! You keep challenging yourself to be at stressful situation. I hope I can be like you some day...Have fun at"$STRESSOR_SOCIAL"! Let me know how it goes!"')
# ###########low neuroticism
# df.add_user_transition(State.SOCIAL_neuroticism, State.SOCIAL_neurotic_no, r'<[!#ONT(ontno)]>')
# df.add_system_transition(State.SOCIAL_neurotic_no, State.SOCIAL_people,
#                          r'[!"Is there anyone you know that is going to be at the"$STRESSOR_SOCIAL"?"]')
#
# ###ERR
# df.set_error_successor(State.SOCIAL_people, State.SOCIAL_people_ERR)
# df.add_system_transition(State.SOCIAL_people_ERR, State.SOCIAL_end,
#                          '"I can tell that you are a very social person. I am sure everyone will be happy to see you"')
#
# df.add_user_transition(State.SOCIAL_people, State.SOCIAL_people_name, '[$social_people=#NER(person)]')
# df.add_system_transition(State.SOCIAL_people_name, State.SOCIAL_end,
#                          r'[!"I can tell that you are a very social person. I am sure "$social_people" will be happy to see you at" $Social_Stressor"."]')
...
# df.add_system_transition(State.SOCIAL_topic, State.SOCIAL_preparedness,
#                          r'[!"Have you thought about some topics you can say at"$STRESSOR_SOCIAL"?"]')
#
...
# df.add_system_transition(State.SOCIAL_prep_yes2, State.SOCIAL_activity,
#                          r'[!"You know" $subject "sounds really interesting to me! You should probably take a break from thinking about" $STRESSOR_SOCIAL ". What is your go-to destress activity besides talking to me?"]')
#
# ########unprepared(likely low conscientiousness)
# df.set_error_successor(State.SOCIAL_preparedness, State.SOCIAL_prep_no)
# df.add_system_transition(State.SOCIAL_prep_no, State.SOCIAL_end,
#                          r'[!"Are you not worried...? Otherwise you must be a natural talker. I really admire that."]')
#
# ####no previous bad social event experience
# df.set_error_successor(State.SOCIAL1, State.SOCIAL2_no)
# df.add_system_transition(State.SOCIAL2_no, State.SOCIAL3,
#                          r'[!"Besides stressed or anxious, what else do you feel about this upcoming" $STRESSOR_SOCIAL"?"]')
# df.add_user_transition(State.SOCIAL3, State.SOCIAL_feeling, '[$feeling=#POS(adj)]')
# df.add_system_transition(State.SOCIAL_feeling, State.SOCIAL_feeling_cont,
#                          r'[!"I usually feel" $feeling "about"$STRESSOR_SOCIAL"too. You know what, sometimes I really enjoy people focus on me. Yeah like Ariana HAHA. Do you sometimes feel that too?"]')
# ######high narci tendency
# df.add_user_transition(State.SOCIAL_feeling_cont, State.SOCIAL_NARCI, r'[!#ONT(ontyes)]')
# df.add_system_transition(State.SOCIAL_NARCI, State.SOCIAL_prep_yes,
#                          r'[!"Wow. It is likely that we are pretty similar type of people. Do you feel ready and prepared for" $STRESSOR_SOCIAL"?"]')
...
#
# df.add_system_transition(State.SOCIAL_CONSCIENT_H, State.SOCIAL_end,
#                          '"Then there is nothing to worry about, just do it! Come back and let me know the good news!"')
# df.add_system_transition(State.SOCIAL_CONSCIENT_L, State.SOCIAL_end,
#                          '"Haha sometimes the best plan is no plan. Come back and let me know the good news!"')
#
# ######low narci tendency
# df.set_error_successor(State.SOCIAL_feeling_cont, State.SOCIAL_NOT_NARCI)
# df.add_system_transition(State.SOCIAL_NOT_NARCI, State.SOCIAL_gain,
#                          r'[!"What do you hope to get out of your experience at this upcoming" $STRESSOR_SOCIAL"then?"]')
#
# ### ERR
# df.set_error_successor(State.SOCIAL_gain, State.SOCIAL_gain_ERR)
# df.add_system_transition(State.SOCIAL_gain_ERR, State.SOCIAL_end,
#                          '"Yes, you have a really good point! I hope that one day I can be as thoughtful as you."')
# df.add_user_transition(State.SOCIAL_gain, State.SOCIAL_value, '[$social_value={#POS(verb,noun)}]')
# df.add_system_transition(State.SOCIAL_value, State.SOCIAL_end,
#                          r'[!"I see. It is amazing that you like to" $social_value". Self-development is important and I hope you meet some quality people that will benefit your self-development at" $STRESSOR_SOCIAL"."]')
# ##
#
# ###Stressor Prompt Error Section
# df.add_system_transition(State.ERR_STRESSOR_UNKNOWN, State.PROMPT_STRESSOR,
#                          r'[! Sorry. I am not familiar with that situation. Tell some something else that you find stressful.]')
